...
class SkillCategoryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Manually Pagination Controls
        paginator = Paginator(skills_with_stats, self.paginate_by)
        page_number = self.request.GET.get('page')
        page_obj = paginator.get_page(page_number)

        if self.request.user.is_authenticated:
            user_teaching_skills = set(OfferedSkill.objects.filter(user = self.request.user,
            skill__category = category,
            is_active = True).values_list('skill_id' , flat=True))
        else:
            user_teaching_skills = ()

        context['skills'] = page_obj
        context['user_teaching_skills'] = set (user_teaching_skills)
...
class OfferedSkillCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        from apps.skill_sessions.models import SkillSwapSession
...
def get_user_stats(request):
    ...
    from apps.skill_sessions.models import SkillSwapSession, SkillSwapRequest
    from django.db.models import Q
